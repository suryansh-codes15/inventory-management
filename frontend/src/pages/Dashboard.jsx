...
const WarehouseMap3D = lazy(() => import("@/components/WarehouseMap3D"));
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get("/reports/low-stock").then((r) => setStockAlerts(r.data)),
  ...
  if (!data) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-center p-6 border border-dashed rounded-2xl bg-card">
        <AlertTriangle className="h-10 w-10 text-destructive mb-3" />
        <h3 className="font-display font-bold text-lg">Failed to load dashboard data</h3>
        <p className="text-sm text-muted-foreground mt-1 max-w-sm">
          There was an error communicating with the server. Please check the backend connection and try again.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 relative" data-testid="dashboard-page">
      ...
      >
        <div className="absolute -top-10 -left-10 w-40 h-40 rounded-full bg-red-500/5 blur-3xl" />
        <div className="flex items-center justify-between mb-4 relative">
          <div>
            <h3 className="font-display font-bold text-lg flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-amber-500" /> Stock Alert Center
            </h3>
            <p className="text-xs text-muted-foreground mt-0.5">Critical stock warnings requiring attention</p>
          </div>
          <div className="flex items-center gap-2">
            <Badge variant="destructive" className="px-2 py-0.5 text-xs">
              {stockAlerts.out_of_stock.length} Out of Stock
            </Badge>
            <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 border-none px-2 py-0.5 text-xs">
              {stockAlerts.low_stock.length} Low Stock
            </Badge>
          </div>
        </div>
        ...
        {stockAlerts.out_of_stock.length === 0 && stockAlerts.low_stock.length === 0 ? (
          <div className="text-sm text-muted-foreground py-8 text-center border border-dashed rounded-xl">
            All products are sufficiently stocked. No alerts at this time.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <div className="text-xs font-semibold uppercase tracking-wider text-red-500 mb-1">
                Critical Stockouts ({stockAlerts.out_of_stock.length})
              </div>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {stockAlerts.out_of_stock.length === 0 ? (
                  <div className="text-xs text-muted-foreground py-4 text-center bg-secondary/20 rounded-lg">No critical stockouts</div>
                ) : (
                  stockAlerts.out_of_stock.map((p) => (
                    <div key={p.id} className="flex items-center justify-between p-2.5 border border-border rounded-lg bg-red-500/5 hover:bg-red-500/10 transition-colors">
                      ...
                      <Badge variant="destructive" className="text-[10px] py-0 px-1.5">OUT</Badge>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="space-y-2">
              <div className="text-xs font-semibold uppercase tracking-wider text-amber-500 mb-1">
                Reorder Warnings ({stockAlerts.low_stock.length})
              </div>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {stockAlerts.low_stock.length === 0 ? (
                  <div className="text-xs text-muted-foreground py-4 text-center bg-secondary/20 rounded-lg">No pending reorders</div>
                ) : (
                  stockAlerts.low_stock.map((p) => (
                    <div key={p.id} className="flex items-center justify-between p-2.5 border border-border rounded-lg bg-amber-500/5 hover:bg-amber-500/10 transition-colors">
                      <div className="min-w-0">
                        <div className="font-medium text-xs truncate">{p.name}</div>
                        <div className="text-[10px] text-muted-foreground font-mono truncate">{p.sku}</div>
                      </div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 border-none text-[10px] py-0 px-1.5">
